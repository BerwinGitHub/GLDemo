...
inline void stackblurJob(unsigned char* src,				///< input image data image->getData();
...
)
{
    ...
    if (step == 1) {
        ...
        for(y = minY; y < maxY; y++) {
            ...
            xp = radius;
            if (xp > wm) xp = wm;
            src_ptr = src + ((xp + y * w) << 2); //   img.pix_ptr(xp, y);
    ...
    if (step == 2) {
        ...
        for(x = minX; x < maxX; x++) {
            ...
            yp = radius;
            if (yp > hm) yp = hm;
            src_ptr = src + ((x + yp * w) << 2); // img.pix_ptr(x, yp);
            dst_ptr = src + (x << 2); 			  // img.pix_ptr(x, 0);
            for(y = 0; y < h; y++) {
                dst_ptr[0] = (sum_r * mul_sum) >> shr_sum;
                dst_ptr[1] = (sum_g * mul_sum) >> shr_sum;
                dst_ptr[2] = (sum_b * mul_sum) >> shr_sum;
